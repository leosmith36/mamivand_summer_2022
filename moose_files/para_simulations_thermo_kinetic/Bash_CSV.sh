...
do
	...
	cp -R build include Makefile scripts test doc lib README.md pf_tm2-opt LICENSE run_tests src ./run_"$i"
    ...
    sed "s/FeCrCo.i/FeCrCo_m22_${m22[$i]}_m33_${m33[$i]}_m23_${m23[$i]}_k_${k[$i]}_lab_${lab[$i]}_lac_${lac[$i]}_lbc_${lbc[$i]}.i/g;s/SAMPLE_JOB/PFTM_$i/g" <../slurm-batch.bash>slurm-batch.bash
