...
def get_df(file,dir,output,img):
    ...
    df = pd.DataFrame(columns=("m22","m33","m23","k","lab","lac","lbc","min","max","file"))
    ...
    with open(dir,"r") as f:
        ...
        kr = float(re.sub("_lab_.*","",re.sub("FeCrCo_m22_.*_m33_.*_m23_.*_k_","",file)))
        labr = float(re.sub("_lac_.*","",re.sub("FeCrCo_m22_.*_m33_.*_m23_.*_k_.*_lab_","",file)))
        lacr = float(re.sub("_lbc_.*","",re.sub("FeCrCo_m22_.*_m33_.*_m23_.*_k_.*_lab_.*_lac_","",file)))
        lbcr = float(re.sub("_c.*","",re.sub("FeCrCo_m22_.*_m33_.*_m23_.*_k_.*_lab_.*_lac_.*_lbc_","",file)))
        ...
        df = pd.DataFrame({"m22":[m22], "m33":[m33], "m23":[m23], "k":[k], "lab":[lab], "lac":[lac], "lbc":[lbc], "min":[min], "max":[max], "file":[img]})
